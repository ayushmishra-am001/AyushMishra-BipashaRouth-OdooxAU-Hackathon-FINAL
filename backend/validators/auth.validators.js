...
function validateVerifyEmail({ token }) {
  if (!token || !token.trim()) throw new AppError('Verification token is required', 400);
}
...
function validateResetPassword({ token, password }) {
  if (!token || !token.trim()) throw new AppError('Reset token is required', 400);
  if (!password || password.length < 6) throw new AppError('Password must be at least 6 characters', 400);
}

module.exports = { validateSignup, validateLogin, validateVerifyEmail, validateForgotPassword, validateResetPassword, parseImageDataUri };
